...
It also mints a short-lived internal service token so this service can perform
service-to-service writes (e.g. registering marts back into the data catalog).
"""
...
def _decode(token: str) -> dict:
    ...
    expected = _b64e(hmac.new(_SECRET.encode(), signing.encode(), hashlib.sha256).digest())
...
def service_headers() -> dict[str, str]:
    """Authorization header carrying the internal service token."""
